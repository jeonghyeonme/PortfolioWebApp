...
8.  **(2025년 9월 16일) 관리자 경험(AX) 개선을 위한 아키텍처 변경:**
    *   **'미러 사이트' 아키텍처 도입:** 기존의 분리된 CMS 대시보드 방식에서 벗어나, 관리자가 방문자용 사이트와 동일한 UI를 보면서 콘텐츠를 관리할 수 있는 '미러 사이트' 아키텍처를 도입했습니다.
    *   **경로 기반 관리자 UI 구현:** `useLocation` 훅을 사용하여 현재 URL 경로가 `/admin`으로 시작하는지 여부에 따라 '수정' 버튼 등 관리자 전용 UI가 동적으로 나타나도록 구현했습니다. 이 방식은 복잡한 전역 상태 관리에 비해 훨씬 안정적이고 예측 가능합니다.
    *   **라우팅 구조 재설계:** `/admin` 경로 아래에 방문자용 사이트의 라우트 구조를 그대로 복제하여, 관리자가 사이트를 둘러보며 직관적으로 작업할 수 있도록 변경했습니다. 기존의 상세 수정/생성 페이지들은 `/admin/manage/` 경로 아래로 재배치했습니다.
    *   **코드베이스 정리:** 새로운 아키텍처 도입에 따라 더 이상 필요 없어진 CMS 대시보드(`DashboardPage.tsx`), `UIKitPage.tsx` 및 불안정한 인라인 편집을 위해 추가되었던 다수의 커스텀 컴포넌트와 훅(`Editable.tsx`, `AuthContext.tsx` 등)을 모두 삭제하여 프로젝트 구조를 단순화했습니다.
    *   **UI/UX 개선:** 관리자 로그인 단축키를 복구하고, 네비게이션 메뉴를 정리했으며, 모든 페이지에 동적 데이터가 포함된 `Footer`가 표시되도록 복구 및 개선했습니다.
...
*   **(핵심) Supabase RLS 정책으로 인한 데이터 접근 문제:**
    *   **문제점:** 비로그인 방문자가 `HeroSection`, `Footer` 등 공용 컴포넌트의 데이터를 볼 수 없는 현상 발생.
    *   **원인:** `Profile` 테이블에 로그인한 사용자만 데이터를 읽을 수 있도록 하는 RLS(Row Level Security) 정책이 설정되어 있었음.
    *   **해결책:** 모든 방문자가 `Profile` 테이블의 데이터를 읽을 수 있도록 허용하는 `FOR SELECT USING (true)` 정책을 SQL로 추가하여 근본적인 문제를 해결.
